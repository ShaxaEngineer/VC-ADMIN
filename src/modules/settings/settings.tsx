...
import { logOut, themeConfigActions, themeConfigSelector } from '@/store';
import { getLocalStorage } from '@/utils';

export const Settings = () => {
  const { t } = useTranslation();
  const [showCustomizer, setShowCustomizer] = useState(false);
  const [timeTocen, setTimeTocen] = useState<string>('00:00:00');

  const dispatch = useAppDispatch();
  const themeConfig = useAppSelector(themeConfigSelector);

  const { toggleAnimation, toggleLayout, toggleMenu, toggleNavbar, toggleTheme, toggleSemidark } =
    themeConfigActions;

  useEffect(() => {
    if (!getLocalStorage(token)) return;
    const intervalId = setInterval(() => {
      const tokenGet = getLocalStorage(token);
      // Tokenni bo'lish
      const parts = tokenGet?.split('.');
      const payload = JSON.parse(atob(parts?.[1])); // Payloadni dekod qilish
      // Amal qilish muddati (exp)
      const expirationTime = payload?.exp; // Unixtimestamp (soniyada)
      // Hozirgi vaqtni olish (millisekundda)
      const currentTime = Math.floor(Date.now() / 1000); // Soniyaga o'girish
      // Token muddatini taqqoslash
      if (currentTime >= expirationTime) {
        setTimeTocen('00:00:00');
        logOut();
      } else {
        const timeLeft = expirationTime - currentTime;
        // Qolgan vaqtni hh:mm:ss formatiga o'zgartirish
        ...
        setTimeTocen(formattedTimeLeft);
      }
    }, 10000);
    ...
    <div>
      ...
      >
        ...
        <div className="perfect-scrollbar h-full overflow-y-auto overflow-x-hidden">
          ...
          {/* tokenning qolgan vaqti */}
          <div className="mb-3 rounded-md border border-dashed border-white-light p-3 dark:border-[#1b2e4b]">
            <h5 className="mb-1 text-base leading-none dark:text-white">{timeTocen}</h5>
            <p className="text-xs text-white-dark">
              {t('Saytda qolish uchun tokenning qolgan vaqti')}
            </p>
          </div>
        </div>
      </nav>
