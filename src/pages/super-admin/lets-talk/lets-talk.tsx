...
export const LetsTalk = () => {
  ...
  const { data, isLoading } = useGet({
    url: buildUrl('letstalk', {
      page: currentPage,
      limit: pageSize,
    }),
    queryKey: [QUERY_KEY.letstalk, currentPage, pageSize],
  });
  // malumotni uzgartish
  ...
  const deleteConfirm = (id: number) => {
    mutate({
      ...
      method: 'DELETE',
      onSuccess: (res: any) => {
        reloadQueryKey([QUERY_KEY.letstalk]);
        ...
        >
          {data?.data?.map((e: any, i: number) => (
            <tr key={i}>
              ...
              <td>{e?.letstalk_fullname}</td>
              <td>{e?.letstalk_email}</td>
              <td>{e?.letstalk_number}</td>
              <td>{e?.letstalk_message}</td>
              <td>
                <Popover
